...
*/
delimiter $ 
drop procedure if exists Proc_Board_SalesTrends_Source $
create procedure Proc_Board_SalesTrends_Source()
begin
    declare iRows int;
    ...
    /*建内存表*/
    create TABLE if not exists Board_SalesTrends_Source (
            dUpdateTime datetime,
            sSeries varchar(255),
            sCategories varchar(255),
            fValue decimal(18,2)); 
    truncate TABLE Board_SalesTrends_Source;  -- 使用前先清空内存表。
    -- 新建日期 
    ...
    begin
        ...
        /*定义游标 以及赋值*/
        declare Cursor_Board_SalesTrends_Source cursor for 
            SELECT
        ...
        /*打开游标*/
        open Cursor_Board_SalesTrends_Source;
        /*循环开始*/
        flag_loop_Board_SalesTrends_Source:loop
        /*给游标变量赋值*/
        fetch Cursor_Board_SalesTrends_Source into sSource; 
        /*判断游标的循环是否结束*/
        if Done then 
            leave flag_loop_Board_SalesTrends_Source ; 
        end if ;
        set j=0;
        while(j<=iRows) do   
            Insert Into Board_SalesTrends_Source (dUpdateTime,sSeries,sCategories,fValue) values (now(),sSource,DATE_FORMAT(DATE_ADD(sDate,INTERVAL j MONTH),'%Y-%m'),0);
            set j = j+1;  
        end while; 
        end loop;  /*循环结束*/
        close Cursor_Board_SalesTrends_Source;/*关闭游标*/
    end;
    -- <循环创建虚拟表记录end>
    
    -- 归类数据并填充
    update Board_SalesTrends_Source set fValue=ifnull((
            SELECT
