...
*/
delimiter $  
drop procedure if exists Proc_Board_PurchaseRanking_Product $ 
create procedure Proc_Board_PurchaseRanking_Product()
begin
    ...
    /*建内存表*/
    create table if not exists Board_PurchaseRanking_Product (
        ItemName        text,
        ItemValue       text); 
    truncate TABLE Board_PurchaseRanking_Product;  -- 使用前先清空内存表。
    ...
    begin
        ...
        /*定义游标 以及赋值*/
        declare Cursor_Board_PurchaseRanking_Product cursor for 
            SELECT
                ifNull(Format((@rowNum := @rowNum + 1), 0), 0) AS rowNo, PurchaseOrders.ItemNo,PurchaseOrders.sName, PurchaseOrders.TotalOrderAmount,PurchaseOrders.OrderQty
                FROM
                (
                    (SELECT
                    IFNULL(SUM(PurchaseOrdersLine.PurchaseAmount * ifNull((Select Rate From Dic_Currency Where CurrencyCode=PurchaseOrders.PurchaseCurrency Limit 0,1),1)), 0) AS TotalOrderAmount, 
                    SUM(PurchaseOrdersLine.OrderQty) AS OrderQty,
                    PurchaseOrdersLine.ItemNo,
                    concat(PurchaseOrdersLine.ENGItemName, '(', PurchaseOrdersLine.ItemNo, ')') as sName, 
                    'RMB' AS Unit
                    FROM
                    PurchaseOrdersLine
                    Left Join PurchaseOrders
                        On PurchaseOrders.rid = PurchaseOrdersLine.pid
                    WHERE PurchaseOrders.OrderDate BETWEEN concat(
                        DATE_SUB(
                    ...
                    GROUP BY PurchaseOrdersLine.ItemNo
        ...
        /*打开游标*/
        open Cursor_Board_PurchaseRanking_Product;
        /*循环开始*/
        flag_loop_PurchaseRanking_Product:loop
        /*给游标变量赋值*/
        fetch Cursor_Board_PurchaseRanking_Product into iRowNo,sItemNo,sName,fTotalAmount,fOrderQty; 
        /*判断游标的循环是否结束*/
        if Done then 
            leave flag_loop_PurchaseRanking_Product ; 
        end if ;
        ...
        set iLastRowNo = (
                        SELECT
                            ...
                            From
                            (SELECT
                                ifNull(Format((@rowNum := @rowNum + 1), 0), 0) AS rowNo, PurchaseOrders.ItemNo, PurchaseOrders.sName, PurchaseOrders.TotalOrderAmount
                                FROM
                                (
                                    (SELECT
                                    IFNULL(SUM(PurchaseOrdersLine.PurchaseAmount * ifNull((Select Rate From Dic_Currency Where CurrencyCode=PurchaseOrders.PurchaseCurrency Limit 0,1),1)), 0) AS TotalOrderAmount, 
                                    PurchaseOrdersLine.ItemNo, concat(
                                        PurchaseOrdersLine.ENGItemName, '(', PurchaseOrdersLine.ItemNo, ')'
                                    ) as sName, 'RMB' AS Unit
                                    FROM
                                    PurchaseOrdersLine
                                    Left Join PurchaseOrders
                                        On PurchaseOrders.rid = PurchaseOrdersLine.pid
                            ...
                            Where TLastPurchaseOrders.ItemNo = sItemNo);
        ...
        insert into Board_PurchaseRanking_Product(ItemName,ItemValue) values (sItemName,sItemValue);
        end loop;  /*循环结束*/
        close Cursor_Board_PurchaseRanking_Product;/*关闭游标*/
    end;
    -- <循环创建虚拟表记录end>  
end $ 
delimiter ;