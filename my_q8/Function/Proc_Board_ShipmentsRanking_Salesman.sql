...
*/
delimiter $ 
drop procedure if exists Proc_Board_ShipmentsRanking_Salesman $
create procedure Proc_Board_ShipmentsRanking_Salesman()
begin
    ...
    /*建内存表*/
    create table if not exists Board_ShipmentsRanking_Salesman (
          ItemName        text,
          ItemValue       text); 
    truncate TABLE Board_ShipmentsRanking_Salesman;  -- 使用前先清空内存表。
    ...
    begin
        ...
        /*定义游标 以及赋值*/
        declare Cursor_Board_ShipmentsRanking_Salesman cursor for 
            SELECT
                ifNull(Format((@rowNum := @rowNum + 1), 0),0) AS rowNo, Shipments.Salesman, Shipments.TotalAmount
                FROM
                (
                    (SELECT
                    IFNULL(Salesman, '') AS Salesman, IFNULL(SUM(TotalAmount) * Shipments.ExchangeRate / (ifNull((Select Rate From Dic_Currency Where CurrencyCode='USD' Limit 0,1),1)), 0) AS TotalAmount
                    FROM
                    Shipments
        ...
        /*打开游标*/
        open Cursor_Board_ShipmentsRanking_Salesman;
        /*循环开始*/
        flag_loop_ShipmentsRanking_Salesman:loop
        /*给游标变量赋值*/
        fetch Cursor_Board_ShipmentsRanking_Salesman into iRowNo,sSalesman,fTotalAmount; 
        /*判断游标的循环是否结束*/
        if Done then 
            leave flag_loop_ShipmentsRanking_Salesman ; 
        end if ;
        ...
        set iLastRowNo = (
                        SELECT
                            ...
                            From
                            (SELECT
                                Format((@rowNum := @rowNum + 1), 0) AS rowNo, Shipments.Salesman, Shipments.TotalAmount
                            FROM
                                (
                                (SELECT
                                    IFNULL(Salesman, '') AS Salesman, IFNULL(SUM(TotalAmount) * Shipments.ExchangeRate / (ifNull((Select Rate From Dic_Currency Where CurrencyCode='USD' Limit 0,1),1)), 0) AS TotalAmount
                            ...
                            Where TLastShipments.Salesman = sSalesman);
        ...
        insert into Board_ShipmentsRanking_Salesman(ItemName,ItemValue) values (sItemName,sItemValue);
        end loop;  /*循环结束*/
        close Cursor_Board_ShipmentsRanking_Salesman;/*关闭游标*/
    end;
    -- <循环创建虚拟表记录end>  
end $ 
delimiter ;