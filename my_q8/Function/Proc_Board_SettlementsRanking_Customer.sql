...
*/
delimiter $ 
drop procedure if exists Proc_Board_SettlementsRanking_Customer $
create procedure Proc_Board_SettlementsRanking_Customer()
begin
    ...
    /*建内存表*/
    create table if not exists Board_SettlementsRanking_Customer (
          ItemName        text,
          ItemValue       text); 
    truncate TABLE Board_SettlementsRanking_Customer;  -- 使用前先清空内存表。
    ...
    begin
        ...
        /*定义游标 以及赋值*/
        declare Cursor_Board_SettlementsRanking_Customer cursor for 
            SELECT
                ifNull(Format((@rowNum := @rowNum + 1), 0),0) AS rowNo, Settlements.CustomerShortName, Settlements.TotalAmount
                FROM
                (
                    (SELECT
                    IFNULL(CustomerShortName, '') AS CustomerShortName, IFNULL(SUM(GrossProfit), 0) AS TotalAmount
                    FROM
                    Settlements
                    WHERE CheckoutDate BETWEEN concat(DATE_SUB(
        ...
        /*打开游标*/
        open Cursor_Board_SettlementsRanking_Customer;
        /*循环开始*/
        flag_loop_SettlementsRanking_Customer:loop
        /*给游标变量赋值*/
        fetch Cursor_Board_SettlementsRanking_Customer into iRowNo,sCustomerShortName,fTotalAmount; 
        /*判断游标的循环是否结束*/
        if Done then 
            leave flag_loop_SettlementsRanking_Customer ; 
        end if ;
        ...
        set fLastTotalAmount=(Select
                                ifNull(SUM(GrossProfit), 0) as TotalAmount
                                From
                                Settlements
                                Where CheckoutDate between concat(DATE_SUB(
                                    DATE_SUB(
        ...
        set iLastRowNo = (
                        SELECT
                            ...
                            From
                            (SELECT
                                Format((@rowNum := @rowNum + 1), 0) AS rowNo, Settlements.CustomerShortName, Settlements.TotalAmount
                            FROM
                                (
                                (SELECT
                                    IFNULL(CustomerShortName, '') AS CustomerShortName, IFNULL(SUM(GrossProfit), 0) AS TotalAmount
                            ...
                            Where TLastSettlements.CustomerShortName = sCustomerShortName);
        ...
        insert into Board_SettlementsRanking_Customer(ItemName,ItemValue) values (sItemName,sItemValue);
        end loop;  /*循环结束*/
        close Cursor_Board_SettlementsRanking_Customer;/*关闭游标*/
    end;
    -- <循环创建虚拟表记录end>  
end $ 
delimiter ;