...
*/
delimiter $ 
drop procedure if exists Proc_Board_ShipmentsRanking_DestinationPort $
create procedure Proc_Board_ShipmentsRanking_DestinationPort()
begin
    ...
    /*建内存表*/
    create table if not exists Board_ShipmentsRanking_DestinationPort (
          ItemName        text,
          ItemValue       text); 
    truncate TABLE Board_ShipmentsRanking_DestinationPort;  -- 使用前先清空内存表。
    ...
    -- <循环创建虚拟表记录begin>
    begin
        declare sDestinationPort varchar(255);
        ...
        /*定义游标 以及赋值*/
        declare Cursor_Board_ShipmentsRanking_DestinationPort cursor for 
            SELECT
                ifNull(Format((@rowNum := @rowNum + 1), 0),0) AS rowNo, Shipments.DestinationPort, Shipments.TotalAmount
                FROM
                (
                    (SELECT
                    IFNULL(DestinationPort, '') AS DestinationPort, IFNULL(SUM(TotalAmount) * Shipments.ExchangeRate / (ifNull((Select Rate From Dic_Currency Where CurrencyCode='USD' Limit 0,1),1)), 0) AS TotalAmount
                    FROM
                    Shipments
        ...
        /*打开游标*/
        open Cursor_Board_ShipmentsRanking_DestinationPort;
        /*循环开始*/
        flag_loop_ShipmentsRanking_DestinationPort:loop
        /*给游标变量赋值*/
        fetch Cursor_Board_ShipmentsRanking_DestinationPort into iRowNo,sDestinationPort,fTotalAmount; 
        /*判断游标的循环是否结束*/
        if Done then 
            leave flag_loop_ShipmentsRanking_DestinationPort ; 
        end if ;
        set fLastTotalAmount = 0;
        set fGlowRate = 0;
        set iLastRowNo = 0;
        if (iRowNo=1) then
           set sItemName = (Select concat('<a class="ui orange circular label">', iRowNo , '</a>' , '&nbsp;' , sDestinationPort)); 
        elseif (iRowNo=2) then
            set sItemName = (Select concat('<a class="ui yellow circular label">', iRowNo , '</a>' , '&nbsp;' , sDestinationPort));
        elseif (iRowNo=3) then
            set sItemName = (Select concat('<a class="ui olive circular label">', iRowNo , '</a>' , '&nbsp;' , sDestinationPort));
        else
            set sItemName = (Select concat('<a class="ui circular label">', iRowNo , '</a>' , '&nbsp;' , sDestinationPort));
        end if;
        -- 去年同期销售额
        set fLastTotalAmount=(Select
                                ...
                                and ShipmentStatus <> '未出运'
                                and DestinationPort = sDestinationPort);
        ...
        set iLastRowNo = (
                        SELECT
                            ...
                            From
                            (SELECT
                                Format((@rowNum := @rowNum + 1), 0) AS rowNo, Shipments.DestinationPort, Shipments.TotalAmount
                            FROM
                                (
                                (SELECT
                                    IFNULL(DestinationPort, '') AS DestinationPort, IFNULL(SUM(TotalAmount) * Shipments.ExchangeRate / (ifNull((Select Rate From Dic_Currency Where CurrencyCode='USD' Limit 0,1),1)), 0) AS TotalAmount
                            ...
                            Where TLastShipments.DestinationPort = sDestinationPort);
        ...
        insert into Board_ShipmentsRanking_DestinationPort(ItemName,ItemValue) values (sItemName,sItemValue);
        end loop;  /*循环结束*/
        close Cursor_Board_ShipmentsRanking_DestinationPort;/*关闭游标*/
    end;
    -- <循环创建虚拟表记录end>  
end $ 
delimiter ;