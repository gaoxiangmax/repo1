...
*/
delimiter $ 
drop procedure if exists Proc_Board_SalesRanking_Customers $
create procedure Proc_Board_SalesRanking_Customers()
begin
    ...
    /*建内存表*/
    create table if not exists Board_SalesRanking_Customers (
        ItemName        text,
        ItemValue       text); 
    truncate TABLE Board_SalesRanking_Customers;  -- 使用前先清空内存表。
    ...
    begin
        ...
        /*定义游标 以及赋值*/
        declare Cursor_Board_SalesRanking_Customers cursor for 
            SELECT
                ifNull(Format((@rowNum := @rowNum + 1), 0),0) AS rowNo, SalesOrders.CustomerShortName, SalesOrders.TotalAmount
            FROM
            (
                (SELECT
                IFNULL(CustomerShortName, '') AS CustomerShortName, IFNULL(SUM(ConvertUSD), 0) AS TotalAmount
                FROM
                SalesOrders
                WHERE OrderDate BETWEEN concat(DATE_SUB(CURDATE(), INTERVAL DAYOFYEAR(CURDATE()) - 1 DAY),' 00:00:00')
                AND concat(CURDATE(),' 23:59:59')
                AND SalesOrderStatus NOT IN ('待确认', '已作废')
                GROUP BY CustomerShortName
                ORDER BY TotalAmount DESC
                LIMIT 0, 10) SalesOrders,
                (SELECT (@rowNum := 0)) tROWS
            );
        /*指定游标循环结束时的返回值 */
        declare continue handler for not found set Done =1; 
        /*打开游标*/
        open Cursor_Board_SalesRanking_Customers;
        /*循环开始*/
        flag_loop_SalesRanking_Customers:loop
        /*给游标变量赋值*/
        fetch Cursor_Board_SalesRanking_Customers into iRowNo,sCustomerShortName,fTotalAmount; 
        /*判断游标的循环是否结束*/
        if Done then 
            leave flag_loop_SalesRanking_Customers ; 
        end if ;
        ...
        set fLastTotalAmount=(Select
                            ...
                            From
                                SalesOrders
                            Where OrderDate between concat(DATE_SUB(DATE_SUB(CURDATE(),INTERVAL dayofyear(CURDATE())-1 DAY), INTERVAL 1 YEAR),' 00:00:00')
        ...
        set iLastRowNo = (
                        ...
                        From  
                        (SELECT
                        Format((@rowNum := @rowNum + 1), 0) AS rowNo, SalesOrders.CustomerShortName, SalesOrders.TotalAmount
                        FROM
                        (
                            (SELECT
                            IFNULL(CustomerShortName, '') AS CustomerShortName, IFNULL(SUM(ConvertUSD), 0) AS TotalAmount
                            FROM
                            SalesOrders
                            WHERE OrderDate BETWEEN concat(DATE_SUB(DATE_SUB(
                                CURDATE(), INTERVAL DAYOFYEAR(CURDATE()) - 1 DAY
                            ), INTERVAL 1 YEAR),' 00:00:00')
                            AND concat(DATE_SUB(CURDATE(), INTERVAL 1 YEAR),' 23:59:59')
                            AND SalesOrderStatus NOT IN ('待确认', '已作废')
                            GROUP BY CustomerShortName
                            ORDER BY TotalAmount DESC
                            ...
                            (SELECT
                            (@rowNum := 0)) tROWS
                        )) TLastSalesOrder Where TLastSalesOrder.CustomerShortName=sCustomerShortName);
        ...
        insert into Board_SalesRanking_Customers(ItemName,ItemValue) values (sItemName,sItemValue);
        end loop;  /*循环结束*/
        close Cursor_Board_SalesRanking_Customers;/*关闭游标*/
    end;
    -- <循环创建虚拟表记录end>
end $ 
delimiter ;