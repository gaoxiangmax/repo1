...
*/
delimiter $
drop trigger if exists Tgr_ComplaintsBlames_AftereUpdate $
create trigger Tgr_ComplaintsBlames_AftereUpdate after update
on ComplaintsBlames 
for each row
begin
    /*定义变量*/
    ...
    set sOldKeyNo=old.PurchaseOrderNo;
    set srid=new.pid;
    ...
    call Proc_Settlements_SumSupplierClaimAmount(sInvoiceNO,sOldKeyNo);-- 结算中心-工厂付款-索赔金额
end$
delimiter ;