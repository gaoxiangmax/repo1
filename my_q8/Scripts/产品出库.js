//--<产品出库_审核并生成库存明细>
var fnDeliveryOrderToStock = function (self, isArchived) {
    var sDeliveryNo = self.recordSet.fieldByName('出库单号').val();
    var sWarehouse = self.recordSet.fieldByName('仓库名称').val();
    var dDate = self.recordSet.fieldByName('出库日期').val();
    ...
    try {
        if (!isArchived) {
            _.db.execute('DeliveryOrder_Delete_Stock', {
                "values": {
            ...
            try {
                oTable.down(function () {
                    var oTxt = {
                        "values": {
                            ...
                            "fInQty": -self.recordSet.fieldByName('产品资料.出库数量').val(),
                ...
                oTable.enableControl('Batch_DeliveryOrder'); //释放子表对象
            }
            _.db.execute('DeliveryOrder_Insert_Stock', aParams);
            _.db.utils.setArchived('产品出库', srid, true);
            _.ui.showInfo(sDeliveryNo + '，' + _.language.get('库存明细已生成！'));
        } else {
            _.ui.showWarnning(sDeliveryNo + '，' + _.language.get('已审核归档，无法生成库存明细！'));
        }
    } catch (err) {
...
function btn_DeliveryOrder_Review(obj) {
    var self = obj.form;
    if (self.moduleName == '产品出库') {
        if (self.addButton) {
            self.addButton("bExtend", _.language.get("扩展"), "", "yelp", true);
            self.button('bExtend').addButton("bDeliveryOrder_Review", "审核并生成库存明细", function () {
                if (self.modified) {
                ...
                var oSQL = _.db.execute('DeliveryOrder_Select_DeliveryOrder', oTxt);
                if (oSQL.length > 0) {
                    ...
                    fnDeliveryOrderToStock(self, isArchived);
                }
            });
        }
    }
}
addListener([EVT_EDITFORM_CREATED, EVT_RECORDSET_AFTER_NEW, EVT_RECORDSET_AFTER_COPY], btn_DeliveryOrder_Review);

//---<产品出库_AfterSave>
function evt_DeliveryOrder_AfterSave(obj) {
    var self = obj.form;
    if (self.moduleName == '产品出库') {
        var isArchived;
        var oTxt = {
...
addListener([EVT_RECORDSET_AFTER_SAVE], evt_DeliveryOrder_AfterSave);

//---<产品出库_BeforeDelete>
function evt_DeliveryOrder_BeforeDelete(obj) {
    return new Promise((resolve, reject) => {
        ...
        if (self.moduleName == '产品出库') {
            try {
                ...
                _.db.utils.queryArchived('产品出库', srid, function (res) {
                    if (res === false) {
                        _.db.execute('DeliveryOrder_Delete_Stock2', {
                            "values": {
                                "srid": srid
                            }
                        });
...
addPromiseListener([EVT_SEARCHFORM_BEFORE_DELETE, EVT_EDITFORM_BEFORE_DELETE], evt_DeliveryOrder_BeforeDelete);

//---<产品出库_DrawCell>
function evt_DeliveryOrder_DrawCell(obj) {
    var self = obj.form;
    ...
    if (self.moduleName == '产品出库') {
        if (obj.rEvent == EVT_FIELD_CHANGED && obj.field.fullName == '产品出库.产品资料.出库数量') {
...
addListener([EVT_EDITFORM_GET_STYLE], evt_DeliveryOrder_DrawCell);